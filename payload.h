...
typedef struct {
   ...
   // Value prediction metadata.
   bool vp_eligible;         // Instruction class is eligible for value prediction.
   bool vp_predicted;        // A value was predicted for this instruction.
   int64_t vp_pred_value;    // Predicted destination value.
   uint64_t vp_confidence;   // Predictor confidence for this prediction.
   bool vp_meas_available;   // A table-based value prediction existed (regardless of confidence).
   int64_t vp_meas_pred_value; // Measured predicted value used for vpmeas_* classification.

   // VPQ slot allocated for this instruction's in-flight prediction.
   bool vpq_allocated;
   uint64_t vpq_index;
   bool vpq_phase;
   ...
   uint64_t inum_stall_cycles; // This is just a measurement of how long a load instruction waited for its inum to become ready after its register operands became ready.
