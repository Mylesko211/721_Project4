...
void pipeline_t::dispatch() {
   ...
   // Clear Store Sets periodically
   if (STORE_SETS) {
      MDP->cyclicClear(cycle);
   }
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      // FIX_ME #9 BEGIN
      // cerr << "ISSUE AT FIX_ME 9" << endl;
      if (PAY.buf[index].C_valid) {
         if (PAY.buf[index].vp_predicted) {
      ...
      if (STORE_SETS && branch_flag) {
         MDP->checkpoint(PAY.buf[index].MDP_RQ_index, PAY.buf[index].MDP_RQ_phase);
      }

      bool inum_valid = false;
      bool inum_ready;
      uint64_t inum = 0;

      if ((STORE_SETS || ORACLE_MDP) && (IS_STORE(PAY.buf[index].flags) || IS_LOAD(PAY.buf[index].flags))) {
         ...
         inum = MDP->predict(inum_valid,
                             PAY.buf[index].MDP_RQ_index,
                             PAY.buf[index].pc,
                             (IS_STORE(PAY.buf[index].flags) ? true : false),
                             PAY.buf[index].AL_index,
                             /* add-ons for oracle */ oracle_addr_valid, oracle_addr, PAY.buf[index].size);
      }
      inum_ready = !inum_valid;
      if (inum_valid) {
         // If the predicted producer is not currently in-flight, or no longer an
         // incomplete store, do not block the load on a wakeup that will never arrive.
         bool inum_active = false;
         if (inum < REN->AL_size) {
            if (REN->AL_head_phase == REN->AL_tail_phase) {
               inum_active = (inum >= REN->AL_head) && (inum < REN->AL_tail);
            }
            else {
               inum_active = (inum >= REN->AL_head) || (inum < REN->AL_tail);
            }
         ...
         if (!inum_active ||
             REN->ActiveList[inum].completed ||
             !REN->ActiveList[inum].store) {
            inum_ready = true;
         }
      }
      ...
      case SEL_IQ:
         ...
         //    * 'inum_valid', 'inum_ready', and 'inum': Valid bit, ready bit, and tag of predicted store dependency (calculated above).
         // 3. As you can see in file pipeline.h, the IQ variable is the Issue Queue itself, NOT a pointer to it.

         // FIX_ME #10a BEGIN
         // cerr << "ISSUE AT FIX_ME 10A" << endl;
         IQ.dispatch(index, DISPATCH[i].branch_mask, PAY.buf[index].lane_id, PAY.buf[index].A_valid, A_ready, PAY.buf[index].A_phys_reg, PAY.buf[index].B_valid, B_ready, PAY.buf[index].B_phys_reg, PAY.buf[index].D_valid, D_ready, PAY.buf[index].D_phys_reg, inum_valid, inum_ready, inum);
         // FIX_ME #10a END
      ...
      case SEL_IQ_NONE:
         ...
         // FIX_ME #10b1 BEGIN
         // cerr << "ISSUE AT FIX_ME 10B1" << endl;
         REN->ActiveList[PAY.buf[index].AL_index].completed = true;
         // FIX_ME #10b1 END
         ...
         if (PAY.buf[index].trap.valid()) {
            ...
            // FIX_ME #10b2 BEGIN
            // cerr << "ISSUE AT FIX_ME 10B2" << endl;
            REN->ActiveList[PAY.buf[index].AL_index].exception = true;
            // FIX_ME #10b2 END
         }
         break;
