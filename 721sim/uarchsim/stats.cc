...
stats_t::stats_t(pipeline_t *_proc) {
   ...
   DECLARE_COUNTER(this, vp_table_bytes, proc);
  ...
  DECLARE_PHASE_RATE(this, mpki_rate, proc, mispredict_count, commit_count, 1000.0);
   ...
   set_phase_interval("nada", 10000); // Disable printing phase counters and rates, by specifying a bogus phase_counter_name, "nada".
   phase_id = 0;
}
...
void stats_t::set_counter(const char *name, uint64_t value) {
   if (counter_map.find(name) != counter_map.end()) {
      counter_map[name]->count = value;
      counter_map[name]->phase_count = value;
   }
}
