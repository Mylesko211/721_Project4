...
void pipeline_t::writeback(unsigned int lane_number) {
   ...
   if (Execution_Lanes[lane_number].wb.valid) {
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////
      if (!PERFECT_VALUE_PRED && PAY.buf[index].vp_eligible && PAY.buf[index].C_valid && PAY.buf[index].vpq_allocated) {
      ...
      if (PAY.buf[index].checkpoint) {
         ...
         if (PERFECT_BRANCH_PRED) {
            ...
            // FIX_ME #15c BEGIN
            // cout << "ISSUE AT FIX_ME 15C" << endl;


            // Branch recovery squashes speculative consumers of predicted values.
            if (!PERFECT_VALUE_PRED) {
               ValuePred->partial_rollback(PAY.buf[index].VPQ_tail, PAY.buf[index].VPQ_tail_phase);
            }
            ...
            // Repair the LFST of the Store Sets memory dependence predictor.
            if (STORE_SETS) {
      ...
      // Detect value mispredictions at writeback and mark the Active List entry.
      // Retirement uses this flag to trigger rollback/squash recovery.
      ...
      // FIX_ME #16 BEGIN
      // cout << "ISSUE AT FIX_ME 16" << endl;
      REN->ActiveList[PAY.buf[index].AL_index].completed = true;
      // FIX_ME #16 END

      // Late memory-dependency wakeup: if a load was dispatched after the store's
      // RR-stage wakeup point, this writeback wakeup prevents a permanent inum stall.
      if (IS_STORE(PAY.buf[index].flags)) {
         IQ.wakeup(PAY.buf[index].AL_index, false);
      }
