...
void issue_queue::dispatch(unsigned int index, unsigned long long branch_mask, unsigned int lane_id,
                           ...
                           bool inum_valid, bool inum_ready, unsigned int inum) {
   unsigned int free;
   ...
   q[free].D_tag = D_tag;
   q[free].inum_valid = inum_valid;
   q[free].inum_ready = inum_ready;
   q[free].inum = inum;
   q[free].A_arrival_time = proc->cycle;
   q[free].B_arrival_time = proc->cycle;
   q[free].D_arrival_time = proc->cycle;
   q[free].inum_arrival_time = proc->cycle;
...
void issue_queue::wakeup(unsigned int tag, bool register_dependency) {
   ...
   for (unsigned int i = 0; i < size && !register_dependency; i++) {
      if (q[i].valid) {
         if (q[i].inum_valid && !q[i].inum_ready && (tag == q[i].inum)) { // Check inum operand.
            q[i].inum_ready = true;
            q[i].inum_arrival_time = proc->cycle;
         }
      }
   }
}
...
void issue_queue::select_and_issue(unsigned int num_lanes, lane *Execution_Lanes) {
   ...
   for (j = 0; j < size; j++) {
      ...
      if (q[i].valid && (!q[i].A_valid || q[i].A_ready) && (!q[i].B_valid || q[i].B_ready) && (!q[i].D_valid || q[i].D_ready) && (!q[i].inum_valid || q[i].inum_ready)) {
         if (PRESTEER) {
         ...
         if (issue) {
            ...
            if (IS_LOAD(proc->PAY.buf[q[i].index].flags)) {
               if (q[i].inum_valid) {
                  uint64_t max_register_operand_arrival_time = 0;
                  if (q[i].A_arrival_time > max_register_operand_arrival_time && q[i].A_valid) {
                     max_register_operand_arrival_time = q[i].A_arrival_time;
                  }
                  if (q[i].B_arrival_time > max_register_operand_arrival_time && q[i].B_valid) {
                     max_register_operand_arrival_time = q[i].B_arrival_time;
                  }
                  if (q[i].D_arrival_time > max_register_operand_arrival_time && q[i].D_valid) {
                     max_register_operand_arrival_time = q[i].D_arrival_time;
                  }
                  proc->PAY.buf[q[i].index].inum_stall_cycles = (q[i].inum_arrival_time > max_register_operand_arrival_time) ? (q[i].inum_arrival_time - max_register_operand_arrival_time) : 0;
               }
               else {
                  proc->PAY.buf[q[i].index].inum_stall_cycles = 0;
               }
            }
...
unsigned int issue_queue::get_length() {
   return length;
}

unsigned int issue_queue::get_inum_blocked_count() {
   unsigned int blocked = 0;
   for (unsigned int i = 0; i < size; i++) {
      if (q[i].valid && q[i].inum_valid && !q[i].inum_ready) {
         blocked++;
      }
   }
   return blocked;
...
void issue_queue::dump_inum_blocked_details(FILE *file, unsigned int limit) {
   ...
   for (unsigned int i = 0; i < size; i++) {
      ...
      unsigned int inum = q[i].inum;
      bool inum_in_range = (inum < proc->REN->AL_size);

      bool al_active = false;
      bool al_store = false;
      bool al_completed = false;

      if (inum_in_range) {
         bool head_p = proc->REN->AL_head_phase;
         bool tail_p = proc->REN->AL_tail_phase;
         uint64_t head = proc->REN->AL_head;
         uint64_t tail = proc->REN->AL_tail;

         if (head_p == tail_p) {
            al_active = (inum >= head) && (inum < tail);
         }
         else {
            al_active = (inum >= head) || (inum < tail);
         }

         al_store = proc->REN->ActiveList[inum].store;
         al_completed = proc->REN->ActiveList[inum].completed;
      }

      unsigned int pay_index = q[i].index;
      uint64_t seq = proc->PAY.buf[pay_index].sequence;
      uint64_t pc = proc->PAY.buf[pay_index].pc;
      unsigned int al_index = proc->PAY.buf[pay_index].AL_index;

      ifprintf(logging_on, file,
               "  IQ[%u] seq=%" PRIu64 " pc=0x%" PRIx64 " AL=%u waits_on_inum=%u in_range=%u active=%u store=%u done=%u\n",
               i, seq, pc, al_index, inum, (unsigned int) inum_in_range,
               (unsigned int) al_active, (unsigned int) al_store,
               (unsigned int) al_completed);
