...
void pipeline_t::retire(size_t &instret) {
   ...
   if (head_valid && completed) { // AL head exists and completed
      ...
      if (!exception && !load_viol) { // No exception, no load violation, and no value misprediction: commit the instruction at the head of the active list.
         if (!PERFECT_VALUE_PRED && PAY.buf[PAY.head].vp_eligible && PAY.buf[PAY.head].C_valid) {
            ValuePred->retire_VPQ(PAY.buf[PAY.head].pc);
         }
         ...
         if (!PAY.buf[PAY.head].split || !PAY.buf[PAY.head].upper) {
            if (!PAY.buf[PAY.head].vp_eligible) {
               ...
               if (!PAY.buf[PAY.head].vp_meas_available) {
                  ...
                  bool pred_correct = (PAY.buf[PAY.head].vp_meas_pred_value == (int64_t) PAY.buf[PAY.head].C_value.dw);
