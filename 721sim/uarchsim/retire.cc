...
void pipeline_t::retire(size_t &instret) {
   ...
   if (head_valid && completed) { // AL head exists and completed
      ...
      if (!exception && !load_viol) { // No exception, no load violation: commit the instruction at the head of the active list.
         if (!PERFECT_VALUE_PRED && PAY.buf[PAY.head].vp_eligible) {
            ValuePred->retire_VPQ(PAY.buf[PAY.head].pc, PAY.buf[PAY.head].C_value.dw);
         }
         ...
         // Value prediction measurement counters are based on retired
         // architectural instructions.
         if (!PAY.buf[PAY.head].split || !PAY.buf[PAY.head].upper) {
            if (!PAY.buf[PAY.head].vp_eligible) {
               inc_counter(vpmeas_ineligible);
            } else {
               inc_counter(vpmeas_eligible);
               // VP stats assume an architectural destination; rename only marks vp_eligible when C_valid.
               assert(PAY.buf[PAY.head].C_valid);
               if (!PAY.buf[PAY.head].vp_meas_available) {
                  inc_counter(vpmeas_miss);
               } else {
                  bool pred_correct =
                      (PAY.buf[PAY.head].vp_meas_pred_value == (int64_t) PAY.buf[PAY.head].C_value.dw);
                  bool pred_confident = PAY.buf[PAY.head].vp_predicted;

                  if (pred_confident && pred_correct) {
                     inc_counter(vpmeas_conf_corr);
                  } else if (pred_confident && !pred_correct) {
                     inc_counter(vpmeas_conf_incorr);
                  } else if (!pred_confident && pred_correct) {
                     inc_counter(vpmeas_unconf_corr);
                  } else {
                     inc_counter(vpmeas_unconf_incorr);
                  }
               }
            }
         }
         ...
         else if (br_misp || val_misp) { //ValuePred Complete-squash the pipeline after committing a mispredicted branch or
