...
typedef struct {
   ...
   // Valid bit, ready bit, and tag of memory dependency (inum).
   bool inum_valid;   // valid bit (inum exists)
   bool inum_ready;   // ready bit (inum is ready)
   unsigned int inum; // inum
   ...
   // Stats only: timestamps for measuring criticality of a load's memory operand.
   uint64_t A_arrival_time;    // set to current cycle when A is woken up (initialize to 0 at dispatch)
   uint64_t B_arrival_time;    // set to current cycle when B is woken up (initialize to 0 at dispatch)
   uint64_t D_arrival_time;    // set to current cycle when D is woken up (initialize to 0 at dispatch)
   uint64_t inum_arrival_time; // set to current cycle when inum is woken up (initialize to 0 at dispatch)
...
public:
   ...
   void wakeup(unsigned int tag, bool register_dependency);
   ...
   void dump_inum_blocked_details(FILE *file = stderr, unsigned int limit = 8);

   unsigned int get_length();
   unsigned int get_inum_blocked_count();
};
