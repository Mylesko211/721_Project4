...
void pipeline_t::rename2() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if (PAY.buf[index].C_valid) {
         ...
         // assert((PAY.buf[index].vp_eligible && ValuePred->is_eligible(is_int_alu, is_fp_alu, is_load)));
         ...
         if (PAY.buf[index].vp_eligible) {
            ...
            if (PERFECT_VALUE_PRED) {
               ...
               if (SVP_ORACLE_CONF) {
                  predicted = false;
                  // Oracle confidence mode only validates a real SVP prediction.
                  if (PAY.buf[index].vp_meas_available && PAY.buf[index].good_instruction) {
                     db_t *actual = get_pipe()->peek(PAY.buf[index].db_index);
                     if (actual->a_num_rdst > 0) {
