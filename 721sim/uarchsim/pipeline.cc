...
#undef OPEN_LOG_FILE
   ...
   /////////////////////////////////////////////////////////////
   if (STORE_SETS) {
      MDP = new StoreSetsMDPWrapper(SSIT_SIZE, LFST_SIZE, sq_size, CLEAR_PERIOD, ALWAYS_PREDICT_CONFLICT);
   }
   else if (ORACLE_MDP) {
      MDP = new OracleMDPWrapper();
   }
   ...
   uint64_t svp_entry_stride_bits_accounting = 0;
   uint64_t svp_entry_instance_ctr_bits_accounting = 0;
   ...
   if (ValuePred) {
      ...
      svp_entry_conf_bits_accounting = bits_for_unsigned(SVP_CONF_MAX);
      svp_entry_retired_value_bits_accounting = 64;
      svp_entry_stride_bits_accounting = 64;
      svp_entry_instance_ctr_bits_accounting = bits_for_unsigned(SVP_Q_SIZE);
   ...
   stats->set_counter("vp_table_bytes", vp_table_bytes);
   ...
   else if (ALWAYS_PREDICT_CONFLICT) {
      fprintf(stats_log, "   Loads synchronize (stall/unstall) at: IQ\n");
      fprintf(stats_log, "   MEMORY DEPENDENCE PREDICTOR: always predict conflict (always speculatively stall)\n");
   }
   else if (STORE_SETS) {
      fprintf(stats_log, "   Loads synchronize (stall/unstall) at: IQ\n");
      fprintf(stats_log, "   MEMORY DEPENDENCE PREDICTOR: Store Sets\n");
   }
   else {
      fprintf(stats_log, "   Loads synchronize (stall/unstall) at: LQ\n");
      ...
      if (unlikely(debug)) {
         ...
         if (MOD(cycle, 0x400000) == 0) {
            ...
            if (num_insn == num_insn_last_beat) {
               /* debug code
               INFO("DEADLOCK.");
               INFO("DEADLOCK details: PAY=%u FQ=%u IQ=%u IQ_inum_blocked=%u LQ=%u SQ=%u",
                    PAY.length,
                    FQ.get_length(),
                    IQ.get_length(),
                    IQ.get_inum_blocked_count(),
                    LSU.get_lq_length(),
                    LSU.get_sq_length());

               INFO("DEADLOCK AL: head=%lu(hp=%u) tail=%lu(tp=%u)",
                    REN->AL_head, (unsigned int) REN->AL_head_phase,
                    REN->AL_tail, (unsigned int) REN->AL_tail_phase);

               if (IQ.get_inum_blocked_count() > 0) {
                  IQ.dump_inum_blocked_details(stderr, 12);
               }
               */
               assert(0);
            }
