...
typedef struct {
   ...
   // For accurate update at retire stage when a memory order violation is detected.
...
public:
   ...
   void get_head_load_info_for_mdp(uint64_t &load_pc, uint64_t &store_pc_violation);

   // STATS
   void set_stats(stats_t *_stats) { this->stats = _stats; }
   void dump_stats(double extra_wait_time_for_inum, FILE *fp);
   ...
   unsigned int get_lq_length() { return lq_length; }
   unsigned int get_sq_length() { return sq_length; }
};
