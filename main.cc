...
static void help() {
   ...
   fprintf(stderr, "  --mdp=<mdp_model>,<mdp_ctr_max>\t<mdp_model>: 0 (always pred. conflict, synchronize through LQ), 1 (always pred. no conflict), 2 (MDP-sticky), 3 (MDP-ctr), 4 (oracle), 5 (store sets), 6 (always pred. conflict, synchronize through IQ). <mdp_ctr_max>: max counter value for MDP-ctr.\n");
   ...
   // team edits
   fprintf(stderr, "  --vp-perf=1\t\tEnable the perfect value predictor.\n");
   fprintf(stderr, "  --vp-svp=<VPQsize>,<oracleconf>,<# index bits>,<# tag bits>,<confmax>\tConfigure the Stride Value Predictor.\n");
   fprintf(stderr, "  --vp-eligible=<predINTALU>,<predFPALU>,<predLOAD>\tConfigure which instruction classes to predict.\n");
...
static void set_mdp_flags(const char *config) {
   ...
   if (sscanf(config, "%lu,%lu", &mdp_model, &mdp_ctr_max) != 2) {
      ...
      fprintf(stderr, "--mdp=<mdp_model>,<mdp_ctr_max>\t<mdp_model>: 0 (always pred. conflict), 1 (always pred. no conflict), 2 (MDP-sticky), 3 (MDP-ctr), 4 (oracle), 5 (store-set), 6 (always stall, synchronize through issue queue). <mdp_ctr_max>: max counter value for MDP-ctr.\n");
      exit(-1);
   }
   else {
      ...
      case 6:
         // Always stall, synchronize through issue queue
         SPEC_DISAMBIG = true;
         MEM_DEP_PRED = true;
         STORE_SETS = true;
         ALWAYS_PREDICT_CONFLICT = true;
         LFST_SIZE = 1;
         SSIT_SIZE = 1;
         break;
...
static void set_store_set_flags(const char *config) {
   ...
   if (sscanf(config, "%lu,%lu,%lu", &ssit_size, &lfst_size, &clear_period) != 3) {
      fprintf(stderr, "Incorrect usage of --store-set=<ssit_size>,<lfst_size>,<clear_period>\n");
      exit(-1);
   }
   else {
      SSIT_SIZE = ssit_size;
...
int main(int argc, char **argv) {
   ...
   parser.option(0, "vp-svp", 1, [&](const char *s) {
      ...
      if (sscanf(s, "%lu,%lu,%lu,%lu,%lu", &qsize, &oracleconf, &index_bits, &tag_bits, &confmax) != 5) {
   ...
   parser.option(0, "vp-eligible", 1, [&](const char *s) {
      ...
      if (sscanf(s, "%lu,%lu,%lu", &predINTALU, &predFPALU, &predLOAD) != 3) {
         fprintf(stderr, "Incorrect usage of --vp-eligible=<predINTALU>,<predFPALU>,<predLOAD>\n");
         exit(-1);
      }
      VP_PRED_INT_ALU = (predINTALU ? true : false);
      VP_PRED_FP_ALU = (predFPALU ? true : false);
