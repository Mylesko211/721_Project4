...
*/
   ...
   if (depth > 0) {
      ...
      if (Execution_Lanes[lane_number].ex[depth - 1].valid) {
         ...
         // FIX_ME #11b BEGIN
         // cout << "ISSUE AT FIX_ME 11B" << endl;
         // test
         if (!IS_LOAD(PAY.buf[index].flags) && !IS_AMO(PAY.buf[index].flags) && PAY.buf[index].C_valid && !PAY.buf[index].vp_predicted) {
            IQ.wakeup(PAY.buf[index].C_phys_reg, true);
            REN->set_ready(PAY.buf[index].C_phys_reg);
         }
         // FIX_ME #11b END
